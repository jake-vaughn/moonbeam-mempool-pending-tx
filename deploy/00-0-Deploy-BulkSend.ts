...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  let mnemonic = "rather over rifle ahead south dismiss law valve exist arrow galaxy season"
  ...
  const artifact = await deployer.loadArtifact("BulkSendTest")

  // Deploy this contract. The returned object will be of a `Contract` type, similarly to ones in `ethers`.
  // `greeting` is an argument for contract constructor.
  const bulkSendContract = await deployer.deploy(artifact, [200000, 99999999])
