...
library FlareLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IFlarePair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(reserveA > 0 && reserveB > 0, "FlareLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(reserveIn > 0 && reserveOut > 0, "FlareLibrary: INSUFFICIENT_LIQUIDITY");
...
contract FlareRouter02 is IFlareRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = FlareLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = FlareLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "FlareRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = FlareLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "FlareRouter: INSUFFICIENT_A_AMOUNT");
    ...
    function removeLiquidityWithPermit(
        ...
        IFlarePair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(token, to, IERC20Flare(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? FlareLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IFlarePair(FlareLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, "FlareRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, "FlareRouter: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            FlareLibrary.pairFor(factory, path[0], path[1]),
    ...
    function swapExactETHForTokens(
        ...
        assert(IWETH(WETH).transfer(FlareLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = FlareLibrary.sortTokens(input, output);
            IFlarePair pair = IFlarePair(FlareLibrary.pairFor(factory, input, output));
            uint256 amountInput;
...

            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = IERC20Flare(input).balanceOf(address(pair)).sub(reserveInput);
                amountOutput = FlareLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, FlareLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20Flare(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Flare(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(FlareLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20Flare(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Flare(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, "FlareRouter: INSUFFICIENT_OUTPUT_AMOUNT");
