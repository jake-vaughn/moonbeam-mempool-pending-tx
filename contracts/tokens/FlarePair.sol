...
contract FlarePair is FlareERC20 {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            if (_msgSender() == migrator) {
                ...
                require(liquidity > 0 && liquidity != uint256(-1), "mint: Bad desired liquidity");
            } else {
    ...
    function swap(
        ...
        require(amount0Out < vars._reserve0 && amount1Out < vars._reserve1, "swap: INSUFFICIENT_LIQUIDITY");

        {
            ...
            if (data.length > 0) IFlareCallee(to).uniswapV2Call(_msgSender(), amount0Out, amount1Out, data);
        ...
        vars.amount1In = vars.balance1 > vars._reserve1 - amount1Out
            ...
            : 0;
        require(vars.amount0In > 0 || vars.amount1In > 0, "swap: INSUFFICIENT_INPUT_AMOUNT");
        {
            // scope for reserve{0,1} - Adjusted, avoids stack too deep errors
            uint256 balance0Adjusted = vars.balance0.mul(10000).sub(vars.amount0In.mul(25));
            uint256 balance1Adjusted = vars.balance1.mul(10000).sub(vars.amount1In.mul(25));
            require(
                balance0Adjusted.mul(balance1Adjusted) >= uint256(vars._reserve0).mul(vars._reserve1).mul(10000**2),
                "swap: K"
            );
        }
    ...
    function skim(address to) external lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, IERC20Flare(_token0).balanceOf(address(this)).sub(reserve0));
        _safeTransfer(_token1, to, IERC20Flare(_token1).balanceOf(address(this)).sub(reserve1));
    }
