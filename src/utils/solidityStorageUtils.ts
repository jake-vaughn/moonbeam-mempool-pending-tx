...
async function getUint256(slot: BytesLike, contractAddress: string | Promise<string>) {
...
async function getShortStr(slot: BytesLike, contractAddress: string | Promise<string>) {
  const paddedSlot = utils.hexZeroPad(slot, 32)
  const storageLocation = await ethers.provider.getStorageAt(contractAddress, paddedSlot)
  const storageValue = BigNumber.from(storageLocation)

  const stringData = utils.toUtf8String(storageValue.and(MaxUint256.sub(255)).toHexString())
  return stringData.replace(/\x00/g, "")
}

async function getLongStr(slot: BytesLike, contractAddress: string | Promise<string>) {
  const paddedSlot = utils.hexZeroPad(slot, 32)
...
async function getArrayItem(slot: BytesLike, contractAddress: string, item: number, byteSize: number) {
...
async function getMappingItem(slot: BytesLike, contractAddress: string, key: BytesLike) {
  const paddedSlot = utils.hexZeroPad(slot, 32)
...
async function getMappingStruct(
  slot: BytesLike,
